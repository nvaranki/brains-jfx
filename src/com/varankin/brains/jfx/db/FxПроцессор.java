...
public final class FxПроцессор extends FxЭлемент<DbПроцессор>
{
    ...
    public FxПроцессор( DbПроцессор элемент ) 
    {
        ...
        ЗАДЕРЖКА   = new FxProperty<>( элемент, "задержка",   КЛЮЧ_А_ЗАДЕРЖКА,   () -> элемент.задержка(),   t -> элемент.задержка( t ) );
        НАКОПЛЕНИЕ = new FxProperty<>( элемент, "накопление", КЛЮЧ_А_НАКОПЛЕНИЕ, () -> элемент.накопление(), t -> элемент.накопление( t ) );
        ПАУЗА      = new FxProperty<>( элемент, "пауза",      КЛЮЧ_А_ПАУЗА,      () -> элемент.пауза(),      t -> элемент.пауза( t ) );
        РЕСТАРТ    = new FxProperty<>( элемент, "рестарт",    КЛЮЧ_А_РЕСТАРТ,    () -> элемент.рестарт(),    t -> элемент.рестарт( t ) );
        СЖАТИЕ     = new FxProperty<>( элемент, "сжатие",     КЛЮЧ_А_СЖАТИЕ,     () -> элемент.сжатие(),     t -> элемент.сжатие( t ) );
        ОЧИСТКА    = new FxProperty<>( элемент, "очистка",    КЛЮЧ_А_ОЧИСТКА,    () -> элемент.очистка(),    t -> элемент.очистка( t ) );
        СТРАТЕГИЯ  = new FxProperty<>( элемент, "стратегия",  КЛЮЧ_А_СТРАТЕГИЯ,  () -> элемент.стратегия(),  t -> элемент.стратегия( t ) );
    }
