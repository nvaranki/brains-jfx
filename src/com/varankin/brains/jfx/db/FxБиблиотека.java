...
public final class FxБиблиотека extends FxЭлемент<DbБиблиотека>
{
    ...
    public FxБиблиотека( DbБиблиотека библиотека ) 
    {
        ...
        ПОЛЯ = buildReadOnlyListProperty( библиотека, "поля", 
            new FxList<>( библиотека.поля(), библиотека, e -> new FxПоле( e ), e -> e.getSource() ) );
        МОДУЛИ = buildReadOnlyListProperty( библиотека, "модули", 
            new FxList<>( библиотека.модули(), библиотека, e -> new FxМодуль( e ), e -> e.getSource() ) );
        РАСЧЕТЫ = buildReadOnlyListProperty( библиотека, "расчеты", 
            new FxList<>( библиотека.расчеты(), библиотека, e -> new FxРасчет( e ), e -> e.getSource() ) );
        ЛЕНТЫ = buildReadOnlyListProperty( библиотека, "ленты", 
            new FxList<>( библиотека.ленты(), библиотека, e -> new FxЛента( e ), e -> e.getSource() ) );
        ПРОЦЕССОРЫ = buildReadOnlyListProperty( библиотека, "процессоры", 
            new FxList<>( библиотека.процессоры(), библиотека, e -> new FxПроцессор( e ), e -> e.getSource() ) );
        КЛАССЫ = buildReadOnlyListProperty( библиотека, "классы", 
            new FxList<>( библиотека.классы(), библиотека, e -> new FxКлассJava( e ), e -> e.getSource() ) );
    }
